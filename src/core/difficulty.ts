...
export type ExerciseDifficultyInput = {
  ...
  approachGroup: Pick<
    ApproachesGroup,
    "sum" | "mean" | "max" | "countTotal" | "countMean"
  >;
  purpose: Purpose;
};
 ...
 */
export function calculateTrainingDifficulty(
  exercises: ExerciseDifficultyInput[],
): number {
  return exercises.reduce(
    (sum, ex) => sum + calculateExerciseDifficulty(ex),
    0,
  );
}
